...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        if is_greedy_gen_mode and use_lascd:
            if generation_config.num_return_sequences > 1:
                raise ValueError(
            ...
            return self.deco_greedy_search(
                ...
                force_1d=force_1d,
                debug=debug,
                layer_select_metric=layer_select_metric or "energy",
                **model_kwargs,
            )
        ...
        elif is_sample_gen_mode and use_lascd:
        ...
        elif is_beam_gen_mode and use_lascd:
    ...
    def _deco_layer_energy(
        ...
        """Compute scalar energy for one layer's attention slice.

        Args:
            attn_last:      [heads, k] attention weights for the last query token.
            image_start_pos: start index of vision tokens in the KV sequence.
            vision_end:      end index (exclusive) of vision tokens.
            laplacian_mode:  "standard" | "sobel" | "log"
                - "standard": For square grids uses the 2-D discrete Laplacian
                              (unless force_1d=True), otherwise 1-D second-order
                              difference. Controlled by force_1d for ablation.
                - "sobel":    approximates gradient magnitude via two 1-D first-
                              order differences (forward & backward), then sums
                              their squared norms — sensitive to edges/transitions.
                - "log":      Laplacian-of-Gaussian approximation: smooth with a
                              3-tap Gaussian [1,2,1]/4 first, then apply standard
                              Laplacian. Reduces noise sensitivity.
            energy_mode:     "vision" | "text"
                - "vision":  compute energy on the vision attention slice.
                - "text":    compute energy on the concatenated text attention
                             slice (everything outside the vision window).
            force_1d:        If True and laplacian_mode=="standard", always use
                             the 1-D second-order difference path even when the
                             vision token count forms a perfect square grid.
                             Has no effect on "sobel" or "log" modes.

        Returns:
            Scalar float energy value for this layer.
        """
        # Select the attention slice according to energy_mode
        if energy_mode == "text":
            signal = torch.cat(
                [attn_last[:, :image_start_pos], attn_last[:, vision_end:]], dim=-1
            )  # [heads, t]
        else:  # "vision" (default)
            signal = attn_last[:, image_start_pos:vision_end]  # [heads, v]

        if signal.shape[1] < 3:
            return 0.0

        if laplacian_mode == "sobel":
            # Forward and backward first-order differences, sum squared norms
            fwd = signal[:, 1:] - signal[:, :-1]
            bwd = signal[:, :-1] - signal[:, 1:]
            e = (torch.norm(fwd, p=2, dim=1) ** 2 + torch.norm(bwd, p=2, dim=1) ** 2)
            return e.mean().item()

        elif laplacian_mode == "log":
            # Gaussian smooth [1,2,1]/4 then standard Laplacian
            g = torch.tensor([0.25, 0.5, 0.25], device=signal.device, dtype=signal.dtype)
            # 1-D conv: [heads, 1, k] -> [heads, 1, k-2] -> [heads, k-2]
            smoothed = F.conv1d(signal.unsqueeze(1), g.view(1, 1, 3), padding=0).squeeze(1)
            if smoothed.shape[1] < 3:
                return 0.0
            lap = smoothed[:, 2:] - 2 * smoothed[:, 1:-1] + smoothed[:, :-2]
            return torch.norm(lap, p=2, dim=1).mean().item()

        else:  # "standard"
            _gs = int(math.sqrt(signal.shape[1]))
            if (not force_1d) and _gs * _gs == signal.shape[1]:
                # 2-D Laplacian on square grid
                _v2d = signal.view(signal.shape[0], 1, _gs, _gs)
                _lap = torch.tensor(
                    [[[[0., 1., 0.], [1., -4., 1.], [0., 1., 0.]]]],
                    device=signal.device, dtype=signal.dtype
                )
                _d2d = F.conv2d(_v2d, _lap, padding=1)
                return torch.norm(_d2d.view(signal.shape[0], -1), p=2, dim=1).mean().item()
            else:
                # 1-D second-order difference (ablation path or non-square grid)
                _d1d = signal[:, 2:] - 2 * signal[:, 1:-1] + signal[:, :-2]
                return torch.norm(_d1d, p=2, dim=1).mean().item()

    @staticmethod
    def _deco_layer_visual_mass(
        attn_last: torch.Tensor,
        image_start_pos: int,
        vision_end: int,
        energy_mode: str,
    ) -> float:
        """Head-averaged attention mass on the slice (sum over positions).

        For ``energy_mode=="vision"``, this matches
        :math:`m^{(\\ell)}=\\|\\frac{1}{H}\\sum_h \\mathbf{A}^{(\\ell,h)}[\\mathcal{I}_v]\\|_1`
        when attention is non-negative. Used as an ablation that replaces Laplacian
        energy for argmax/argmin layer selection only; logits remapping is unchanged.
        """
        if energy_mode == "text":
            signal = torch.cat(
                [attn_last[:, :image_start_pos], attn_last[:, vision_end:]], dim=-1
            ...
            signal = attn_last[:, image_start_pos:vision_end]
        if signal.numel() == 0:
            return 0.0
        alpha_mean = signal.mean(dim=0)
        return float(alpha_mean.sum().item())

    @staticmethod
    def _deco_layer_logits_entropy(logits_last_token: torch.Tensor, top_k: int = 10) -> float:
        """对单个位置的 logits 取 top-``k`` 后做 softmax 的 Shannon 熵（自然底，nats）。"""
        if logits_last_token.dim() != 1:
            logits_last_token = logits_last_token.view(-1)
        vocab = logits_last_token.shape[0]
        if vocab == 0:
            return 0.0
        k = min(int(top_k), int(vocab))
        if k <= 0:
            return 0.0
        top_scores, _ = torch.topk(logits_last_token, k)
        probs = F.softmax(top_scores, dim=-1)
        plogp = probs * torch.log(probs + 1e-12)
        return float(-(plogp.sum()).item())

    def _deco_select_premature_and_gt_layers(
        self,
        layer_select_metric: str,
        early_exit_layers: List[int],
        dict_outputs: Dict[int, torch.Tensor],
        outputs,
        input_ids: torch.LongTensor,
        _original_input_len: int,
        _num_vision_tokens: int,
        _kv_len: int,
        laplacian_mode: str,
        energy_mode: str,
        force_1d: bool,
        ...
        """在候选层中选出“幻觉/过早”层与 GT 层。

        - ``energy``：**能量 larger → 视为 hallu 层，smaller → GT 层**（Laplacian 注意力能量）。
        - ``visual_mass``：**质量 larger → 视为 GT 层，smaller → hallu 层**（注意力质量越大越像真实视觉证据）。
        - ``entropy``：**熵最大 → 幻觉最大层，熵最小 → GT 层**（由各早退层在上一 token 位置的 top-k 分布熵比较）。

        Returns:
            (premature_layer, gt_layer, value_at_premature, value_at_gt, metric_name)
        """
        default = early_exit_layers[0]
        if not early_exit_layers:
            return default, default, 0.0, 0.0, "none"

        if layer_select_metric == "entropy":
            if dict_outputs is None or len(dict_outputs) == 0:
                return default, default, 0.0, 0.0, "entropy"
            _max_e = -1.0
            _min_e = float("inf")
            _argmax_l = default
            _argmin_l = default
            for _layer_idx in early_exit_layers:
                if _layer_idx not in dict_outputs:
                    continue
                d = dict_outputs[_layer_idx]
                if seq_idx is None:
                    _lv = d[0, -1, :]
                else:
                    _lv = d[seq_idx, -1, :]
                _e = self._deco_layer_logits_entropy(_lv, top_k=entropy_top_k)
                if _e > _max_e:
                    _max_e = _e
                    _argmax_l = _layer_idx
                if _e < _min_e:
                    _min_e = _e
                    _argmin_l = _layer_idx
            if _min_e is float("inf"):
                _min_e = 0.0
            if _max_e < 0.0 and _argmax_l == default:
                _max_e = 0.0
            # 熵最大 = premature / 幻觉，熵最小 = GT
            return _argmax_l, _argmin_l, _max_e, _min_e, "entropy"

        mname = "mass" if layer_select_metric == "visual_mass" else "energy"
        _max_m = -1.0
        _min_m = float("inf")
        _argmax_l = default
        _argmin_l = default
        if outputs.attentions is None or _num_vision_tokens <= 0:
            return default, default, 0.0, 0.0, mname

        _image_start_pos, _vision_end = self._deco_get_vision_slice_bounds(
            input_ids, _original_input_len, _num_vision_tokens, _kv_len, outputs
        )
        for _layer_idx in early_exit_layers:
            if _layer_idx < len(outputs.attentions):
                if seq_idx is None:
                    _attn_last = outputs.attentions[_layer_idx][0, :, -1, :]
                else:
                    _attn_last = outputs.attentions[_layer_idx][seq_idx, :, -1, :]
                if layer_select_metric == "visual_mass":
                    _hf_m = self._deco_layer_visual_mass(
                        _attn_last, _image_start_pos, _vision_end, energy_mode
                    )
                else:
                    _hf_m = self._deco_layer_energy(
                        _attn_last,
                        _image_start_pos,
                        _vision_end,
                        laplacian_mode,
                ...
                if _hf_m < _min_m:
                    _min_m = _hf_m
                    _argmin_l = _layer_idx
        if _min_m is float("inf"):
            _min_m = 0.0
        if _max_m < 0.0 and _argmax_l == default and _argmin_l == default:
            _max_m = 0.0
        if layer_select_metric == "visual_mass":
            # visual_mass: argmax → GT，argmin → hallu (premature)
            return _argmin_l, _argmax_l, _min_m, _max_m, mname
        # energy: argmax → hallu (premature)，argmin → GT
        return _argmax_l, _argmin_l, _max_m, _min_m, mname

    def _deco_get_vision_slice_bounds(
        self,
        input_ids: torch.LongTensor,
        ...
        """Return [start, end) indices over the attention key sequence for vision tokens.

        If the LM has ``_selfcd_vision_span_fn`` (set by SelfCD / callers), it is invoked as::

            fn(model, input_ids, original_input_len, num_vision_tokens, kv_seq_len, outputs) -> (start, end)

        Otherwise fall back to LLaVA-style ``IMAGE_TOKEN_INDEX == -200`` plus ``num_vision_tokens``.
        """
        fn = getattr(self, "_selfcd_vision_span_fn", None)
        if callable(fn):
            return fn(self, input_ids, original_input_len, num_vision_tokens, kv_seq_len, outputs)
        _IMAGE_TOKEN_INDEX = -200
        _img_pos_match = (input_ids[0] == _IMAGE_TOKEN_INDEX).nonzero(as_tuple=True)[0]
        _image_start_pos = _img_pos_match[0].item() if len(_img_pos_match) > 0 else 0
        _vision_end = _image_start_pos + num_vision_tokens
        return _image_start_pos, _vision_end
    ...
    def deco_greedy_search(
        ...
        threshold_top_k: int,
        early_exit_layers: List[int],
        laplacian_mode: str = "standard",
        beta: float = 0.0,
        energy_mode: str = "vision",
        force_1d: bool = False,
        debug: bool = False,
        layer_select_metric: str = "energy",
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        max_length: Optional[int] = None,
        pad_token_id: Optional[int] = None,
        ...
        **model_kwargs,
        ...
        if layer_select_metric not in ("energy", "visual_mass", "entropy"):
            raise ValueError(
                f"layer_select_metric must be 'energy', 'visual_mass' or 'entropy', got {layer_select_metric!r}"
            )
        ...
        _original_input_len = input_ids.shape[1]
        _num_vision_tokens = 0
        _is_first_step = True
        ...
        while True:
            ...
            _kv_len = outputs.attentions[0].shape[-1] if outputs.attentions is not None and len(outputs.attentions) > 0 else 0
            if _is_first_step and outputs.attentions is not None and len(outputs.attentions) > 0:
                _num_vision_tokens = _kv_len - _original_input_len + 1
            ...
            _min_hf_layer, _max_hf_layer, _hallu_metric_val, _gt_metric_val, _mname = self._deco_select_premature_and_gt_layers(
                layer_select_metric,
                early_exit_layers,
                dict_outputs,
                outputs,
            ...
            # energy: argmax→hallu, argmin→GT；visual_mass: argmin→hallu, argmax→GT；entropy: argmax→hallu, argmin→GT
            selected_premature_layer_logits = dict_outputs[_min_hf_layer][:, -1, :]
            ...
            next_token_logits = outputs.logits[:, -1, :]
            gt_logits = dict_outputs[_max_hf_layer][:, -1, :]
            _gt_max_prob = torch.softmax(gt_logits, dim=-1).max(dim=-1).values.mean().item()
            _beta_eff = beta * _gt_max_prob
            if debug:
                print(
                    f"[DECO-DEBUG] hallu_layer={_min_hf_layer} {_mname}={_hallu_metric_val:.4f} | "
                    f"gt_layer={_max_hf_layer} {_mname}={_gt_metric_val:.4f} | "
                    f"beta_eff={_beta_eff:.4f}"
                )
            final_token_logits = (1 + alpha) * next_token_logits - alpha * selected_premature_layer_logits + _beta_eff * gt_logits
            # final_token_logits = next_token_logits - alpha * selected_premature_layer_logits
        ...
        **model_kwargs,
        ...
        while True:
            ...
            for seq_idx in range(multi_candidate_tokens_cumulative_probs.size(0)):
                ...
                # energy: max→hallu, min→GT；visual_mass: min→hallu, max→GT；entropy: max→hallu, min→GT.
                selected_premature_layer_logits = dict_outputs[_min_hf_layer][seq_idx, -1, :]
                indices_to_remove = torch.ones_like(selected_premature_layer_logits)
                indices_to_remove[candidate_tokens_ids] = 0
                indices_to_remove = indices_to_remove.bool()
                next_token_logits = outputs.logits[seq_idx, -1, :]
                gt_logits = dict_outputs[_max_hf_layer][seq_idx, -1, :]
                _gt_max_prob = torch.softmax(gt_logits, dim=-1).max().item()
                _beta_eff = beta * _gt_max_prob
                if debug:
                    print(
                        f"[DECO-DEBUG] seq={seq_idx} hallu_layer={_min_hf_layer} {_mname}={_hallu_metric_val:.4f} | "
                        f"gt_layer={_max_hf_layer} {_mname}={_gt_metric_val:.4f} | "
                        f"beta_eff={_beta_eff:.4f}"
                    )
                final_token_logits = (1 + alpha) * next_token_logits - alpha * selected_premature_layer_logits + _beta_eff * gt_logits
